...
const Sidebar = ({ isOpen, isDesktopExpanded, toggleSidebarMobile }) => {
  ...
  // --- LOGIC 1: LOGOUT (KELUAR) ---
  const handleLogout = (e) => {
    e.preventDefault(); 
    
    // Tambahkan konfirmasi biar aman
    const confirmLogout = window.confirm("Apakah Anda yakin ingin keluar dari sistem?");
    
    if (confirmLogout) {
        // Hapus SEMUA data di localStorage (Token, Username, dll) biar bersih
        localStorage.clear();
  ...
  // --- LOGIC 2: SEARCH (PENCARIAN) ---
  const handleSearch = (e) => {
      // Jika dipanggil dari form/enter, cegah reload
      if (e) e.preventDefault();

      if (searchQuery.trim()) {
          // Arahkan ke halaman Data Barang dengan membawa state pencarian
          // Nanti di halaman Data Barang kita tangkap state ini
          navigate('/data-barang', { state: { globalSearch: searchQuery } });
          
          // Opsional: Reset search bar setelah mencari
          // setSearchQuery(''); 
      }
  };
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1 no-scrollbar">
            ...
            <div className={`flex items-center mb-6 transition-all duration-300 ${!isDesktopExpanded && 'hidden'}`}>
                <input 
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleSearch(e)} // Enter untuk cari
                ...
                <button 
                    ...
                    className="bg-simig-input text-gray-400 px-3 py-2.5 rounded-r-md border-l border-gray-600 hover:text-white hover:bg-gray-700 transition h-full flex items-center"
                >
            ...
            <nav className="space-y-2">
                ...
                <button 
                    ...
                    className="w-full flex items-center px-4 py-3 text-simig-text hover:text-red-400 hover:bg-simig-input rounded-lg transition-colors focus:outline-none"
                >
