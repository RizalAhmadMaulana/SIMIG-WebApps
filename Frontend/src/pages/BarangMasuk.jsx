...
const BarangMasuk = () => {
  ...
  const handleProcessCetak = () => {
      ...
      try {
          ...
          // Header
          doc.setFontSize(18);
          doc.text(`Laporan Barang Masuk - ${namaBulan}`, 14, 20);
          doc.setFontSize(10);
  ...
  return (
    <DashboardLayout>
                ...
                <table className="w-full text-sm text-left border-collapse border border-gray-300">
                    ...
                    <tbody className="text-gray-600">
                        {loading ? (
                            ...
                            <tr><td colSpan="6" className="py-4 text-center">Belum ada transaksi barang masuk.</td></tr>
                        ) : (
        ...
        <Modal 
            ...
            onClose={closeModal} 
            title={<>{isEdit ? <Edit className="w-5 h-5" /> : <Plus className="w-5 h-5" />} {isEdit ? "Edit Barang Masuk" : "Tambah Barang Masuk"}</>}
            size="max-w-lg"
        >
        ...
        <Modal 
            ...
            title={<><Printer className="w-5 h-5" /> Cetak Barang Masuk</>}
            size="max-w-md"
        >
            <div className="space-y-4">
                ...
                <div className="flex justify-end items-center mt-6 pt-4 border-t border-gray-200 gap-3">
                    <button 
                        onClick={() => setBulanSelected("")} 
                        className="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded shadow flex items-center gap-2"
                    >
                        <Trash2 className="w-4 h-4" /> Reset
                    </button>
                    <button 
                        onClick={handleProcessCetak} 
                        className="bg-simig-light hover:bg-blue-600 text-white font-medium py-2 px-4 rounded shadow flex items-center gap-2 transition-colors"
                    >
